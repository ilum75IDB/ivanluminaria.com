...
- Nodi: mysql-node-01, mysql-node-02, mysql-node-03
- mysql-node-03 e' il nodo che ha fatto il bootstrap (`--wsrep-new-cluster`)

## Cosa e' successo
- Il nodo **mysql-node-01** e' andato giu' brevemente e si e' riavviato alle 10:51 del 10 marzo 2026
...
5. `SHOW STATUS LIKE 'wsrep_cluster_state_uuid'` — stesso UUID su tutti (`<uuid-del-cluster>`)
...
|------------|-------------------|-------|---------|-------------------------------|
| mysql-node-03  | 27/02 15:34       | 58    | 45.0G   | Bootstrap node                |
| mysql-node-02  | 27/02 15:35       | 16    | 37.1G   | Meno traffico? Da verificare  |
| mysql-node-01  | 10/03 10:51       | 48    | 12.6G   | Riavviato oggi, rientrato OK  |

## Osservazione
mysql-node-02 ha significativamente meno task (16 vs 58/48) — potrebbe non ricevere traffico dal load balancer, oppure semplicemente meno connessioni attive in quel momento.
