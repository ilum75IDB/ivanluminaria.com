...
No era la primera vez. Desde hacía unas semanas el proceso de carga nocturno iba retrasándose — primero tres horas y media, luego casi cuatro, luego más. La ventana batch estaba fijada entre las 23:00 y las 6:30, y el sistema había empezado a pasarse regularmente. Al día siguiente nos sentamos con el DBA del cliente delante de los logs y empezamos a ver qué estaba pasando de verdad.
...
El primer instrumento que usamos fue AWR [1]. Un informe AWR sobre la ventana nocturna mostraba enseguida dónde iba el tiempo: el top SQL por elapsed time era un bloque PL/SQL con un cursor que iteraba fila a fila sobre 15 millones de registros.
...
Tres líneas de código, tres causas de lentitud. Vamos por orden.

## Las cuatro causas de un ETL que no llega

**1. INSERT fila a fila (row-by-row = slow-by-slow)**

Es una de las frases más citadas en los cursos de Oracle, pero sigue apareciendo en producción. Cada `INSERT` individual genera un round-trip hacia el buffer cache, actualiza los segmentos de undo y escribe en el redo log. Multiplicado por 15 millones de filas, el coste de contexto por cada operación individual acaba dominando sobre el coste del dato en sí.
...
AWR mostraba `dim_customer` como la tabla con el mayor número de logical reads en toda la ventana nocturna. No era casualidad.
...
El primer paso fue separar la carga de la transformación. Los datos de la fuente se cargan primero en una staging table con un `INSERT /*+ APPEND */ ... SELECT` — una única operación bulk que bypasea el buffer cache y escribe directamente en los datafiles (direct path insert) [2].
...
Tras la creación, los lookups sobre 2,8 millones de filas pasaron a ser index range scan sobre una columna con alta selectividad. El coste por lookup se desplomó.
...
Sin esta instrucción, los hints `PARALLEL` sobre operaciones DML se ignoran en silencio — un detalle que también nos hizo perder tiempo la primera vez que probamos la reescritura y no veíamos mejoras significativas.
...
| COMMIT totales | ~150.000 | 2 (staging + MERGE) |
| Procesos paralelos activos | 1 | 8 |
| Redo generado | ~18 GB | ~3,2 GB |

El redo generado bajó también gracias al `NOLOGGING` sobre la staging table — que hay que usar con criterio: una staging table `NOLOGGING` no es recuperable desde un backup incremental tomado durante la carga. En nuestro caso era aceptable porque la staging se recrea desde cero cada noche a partir de la fuente.

El proceso de carga ahora termina a las 00:24. La ventana batch vuelve a tener margen de sobra.

## Lo que vale la pena llevarse

Unas semanas después de la puesta en producción, el DBA del cliente nos mandó otro mensaje — esta vez menos lacónico: "Funciona. Gracias."

Lo que aprendimos (o mejor dicho, confirmamos) en este proyecto no es nuevo, pero merece escribirse de forma explícita porque sigue apareciendo una y otra vez:

**El row-by-row es el asesino silencioso de los ETL legacy.** No es obvio hasta que se mira AWR o un trace 10046. El código parece razonable — un loop, un insert, un commit. El problema es que "razonable" no significa "eficiente" cuando se escala a millones de filas.

**El COMMIT frecuente no protege: ralentiza.** Si el proceso debe poder reanudarse en caso de error, la estrategia correcta es la staging table con un flag de estado — no el commit cada N filas sobre la tabla de destino.

**El paralelismo en Oracle requiere configuración explícita.** `ALTER SESSION ENABLE PARALLEL DML` no es opcional si se quieren operaciones DML paralelas. Y los grados de paralelismo hay que calibrarlos sobre el servidor real, no elegirlos al azar.
...
El patrón — staging table → transformación con joins indexados → MERGE bulk con parallel DML → commit único — es reutilizable en cualquier ETL Oracle con características similares. No es una solución mágica: requiere entender el perfil del dato (cardinalidad, distribución, frecuencia de actualización) y probar los grados de paralelismo sobre el hardware real. Pero como punto de partida para una reescritura, funciona.
...
- **[Direct Path Insert](/es/glossary/direct-path-insert/)** — Modalidad de INSERT en Oracle (activada por el hint `APPEND`) que bypasea el buffer cache y escribe directamente en los datafiles. Reduce drásticamente el coste de carga bulk, pero requiere atención a la estrategia de backup y recovery.
...
- **[Parallel DML](/es/glossary/awr/)** (Oracle) — Ejecución paralela de operaciones DML (INSERT, UPDATE, DELETE, MERGE) sobre múltiples procesos Oracle. Requiere `ALTER SESSION ENABLE PARALLEL DML` y hints explícitos. Sin la habilitación a nivel de sesión, los hints se ignoran en silencio.

- **[Staging table](/es/glossary/direct-path-insert/)** — Tabla temporal usada como área de aterrizaje de los datos en bruto antes de la transformación y la carga en el destino final. Permite separar las fases del ETL, gestionar la reanudabilidad y aplicar transformaciones en bulk en lugar de fila a fila.
