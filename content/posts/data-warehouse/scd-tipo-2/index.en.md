...
seoTitle: "SCD Type 2: Slowly Changing Dimensions with surrogate keys"
description: "SCD Type 2 in the data warehouse: historicising dimensions with surrogate keys and validity dates. Real case: a customer dimension that evolves over time."
...
This is what {{< glossary term="kimball" >}}Kimball{{< /glossary >}} calls **SCD Type 1** — Slowly Changing Dimension Type 1 [1]. Overwrite the old value with the new one. No history, no versioning. The current value erases the previous one.
...
Type 2 doesn't overwrite. It versions [2].
...
1. **A {{< glossary term="chiave-surrogata" >}}surrogate key{{< /glossary >}}** — an identifier generated by the DWH, distinct from the source system's natural key [3]. This is needed because the same customer will have multiple records (one per version), so the natural key is no longer unique.
