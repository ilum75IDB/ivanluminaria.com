---
title: "Know-How & Impact"
description: "Experiencia, profundidad técnica e impacto medible en los sistemas y en los resultados del negocio."
date: "2026-02-23"
lastmod: "2026-02-26"
draft: false
layout: "simple"
image: "resumes.cover.jpg"
---

En casi treinta años de trabajo no he cambiado de profesión.

He cambiado de profundidad.

Cada rol que he asumido no ha sido un movimiento lateral.  
Ha sido una profundización vertical.

Lo que hoy ofrezco no es una lista de competencias.  
Es la suma de las consecuencias que esas competencias generan.

---

## Data Warehouse Architect

Mi experiencia en Data Warehouse se ha desarrollado en entornos donde el dato es infraestructura crítica.

He trabajado para grandes operadores Telco como TIM, Wind, Vodafone y 3, en proyectos internacionales con Huawei,  
en el sector asegurador con Generali,  
en entornos institucionales como el Banco de Italia, la Oficina Italiana de Cambios y Cassa Depositi e Prestiti,  
en el sector farmacéutico,  
y en el sector automotriz con Rover y Alfa Romeo.

Sectores distintos. Una misma exigencia: fiabilidad.

En Telco importa la escala.  
En banca e instituciones públicas importa la precisión normativa.  
En seguros importa la coherencia del riesgo.  
En el sector farmacéutico importa el cumplimiento regulatorio.  
En automoción importa la sincronización de procesos y la calidad del dato a lo largo de la cadena de suministro.

He diseñado arquitecturas de datos en entornos donde el margen de error no existe.

👉 [Leer CV online](/es/resumes/dwh-architect/) | [__Descarga la roadmap de mis competencias y experiencia como Data Warehouse Architect__]({{% staticurl "downloads/CV-IvanLuminaria-DWHArchitect(English).pdf" %}})

---

## AI Manager – Data & Intelligence Strategy

La Inteligencia Artificial no es una moda. Es una evolución.

Después de casi tres décadas diseñando arquitecturas de datos y gobernando sistemas mission-critical, la IA representa la extensión natural del dato hacia la inteligencia.

Un AI Manager no es quien entrena un modelo.  
Es quien gobierna el impacto de la IA en la organización.

Esto significa:

– Evaluar dónde la IA genera valor real y dónde solo genera entusiasmo.  
– Integrar modelos en arquitecturas existentes sin comprometer estabilidad ni seguridad.  
– Garantizar coherencia entre gobierno del dato, cumplimiento normativo y automatización.  
– Traducir necesidades de negocio en soluciones inteligentes y sostenibles.

En sectores regulados como Banca, Seguros, Farmacéutico y Administración Pública, la IA requiere responsabilidad y visión estratégica.

La IA no es magia.  
Es arquitectura aplicada a la inteligencia.

---

## Project Manager con background técnico

Mi experiencia en gestión de proyectos ha madurado coordinando iniciativas en entornos complejos:

Telecomunicaciones internacionales,
Instituciones financieras centrales,
Seguros,
Farmacéutico,
Automoción y Administración Pública.

En estos sectores, el error tiene un coste real.

Un Project Manager técnico comprende el impacto de las decisiones.
Sabe cuándo un compromiso es aceptable.
Y cuándo no lo es.

Conecto visión estratégica con detalle técnico.
Roadmaps con implementación.
Decisiones con consecuencias.

👉 [Leer CV online](/es/resumes/project-manager/) | [__Descarga la roadmap de mis competencias y experiencia como Project Manager__]({{% staticurl "downloads/CV-IvanLuminaria-ProjectManager(ENG).pdf" %}})

---

## Oracle DBA & Performance Tuning Expert

Mi actividad como Oracle DBA se ha consolidado en entornos mission-critical para operadores como TIM, Wind, Vodafone y 3,
en contextos tecnológicos complejos como Huawei,
para instituciones financieras como el Banco de Italia, la Oficina Italiana de Cambios y Cassa Depositi e Prestiti,
para organizaciones aseguradoras como Generali,
y en proyectos industriales en el sector automotriz.

Sistemas de alta disponibilidad.
Cargas elevadas.
Responsabilidad real de guardia.

Aquí el tuning no es una mejora estética.
Es protección operativa.

👉 [Leer CV online](/es/resumes/oracle-dba/) | [__Descarga la roadmap de mis competencias y experiencia como Oracle DBA__]({{% staticurl "downloads/CV-IvanLuminaria-OracleDBAPerformanceTuningExpert(ENG).pdf" %}})

---

## Oracle PL/SQL – Senior & Mentor

He desarrollado PL/SQL en entornos Telco, Banca, Seguros, Automoción, Farmacéutico y Administración Pública.

En contextos como Generali o el Banco de Italia, la lógica en base de datos es parte de la estabilidad del sistema.
En automoción, la coherencia del dato impacta directamente en los procesos productivos y en la cadena de suministro.

Durante años he enseñado SQL y PL/SQL, contribuyendo al crecimiento técnico de desarrolladores que hoy trabajan sobre sistemas complejos.

Hoy no me limito a escribir código.
Puedo liderar.
Puedo hacer mentoring.
Puedo ayudar a desarrolladores a crecer en profundidad técnica y arquitectónica.

👉 [Leer CV online](/es/resumes/oracle-plsql/) | [__Descarga la roadmap de mis competencias y experiencia como Oracle PL/SQL Developer__]({{% staticurl "downloads/CV-IvanLuminaria-OraclePLSQLDeveloper(ENG).pdf" %}})
