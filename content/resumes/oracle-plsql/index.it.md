...
Oltre a PL/SQL e SQL riesco a leggere, comprendere e modificare anche altri linguaggi — Unix Shell Script, Python, Java, C++, JavaScript, HTML, CSS, Pascal e Camel. Alcuni li ho studiati all'università, altri imparati quando è servito toccare il codice attorno al database. Non sono il guru di nessuno di loro; ma quando un pezzo di logica critica vive fuori dal database — in uno script shell, in un servizio Python, in un frontend che chiama procedure PL/SQL — leggere l'altro lato aiuta a capire dove sta davvero il problema.
